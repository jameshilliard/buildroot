...
QT5WEBENGINE_DEPENDENCIES = qt5declarative qt5webchannel host-bison host-flex \
	host-gperf host-harfbuzz host-icu host-ninja host-nodejs host-pkgconf host-python host-webp host-zlib
...
define QT5WEBENGINE_CREATE_HOST_PYTHON_WRAPPER
	mkdir -p $(@D)/host-bin
	sed s%@HOST_DIR@%$(HOST_DIR)%g $(QT5WEBENGINE_PKGDIR)/host-python-wrapper.in > $(@D)/host-bin/python2
	chmod +x $(@D)/host-bin/python2
endef
...
QT5WEBENGINE_ENV += \
	...
	GN_HOST_TOOLCHAIN_EXTRA_CPPFLAGS="$(HOST_CPPFLAGS) -DU_DISABLE_RENAMING=1"
